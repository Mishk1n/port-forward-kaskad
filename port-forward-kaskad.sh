...
# --- ПОДГОТОВКА СИСТЕМЫ ---
prepare_system() {
    # Автоматическое создание глобальной команды kaskad
    if [ "$0" != "/usr/local/bin/kaskad" ]; then
        cp -f "$0" "/usr/local/bin/kaskad"
        chmod +x "/usr/local/bin/kaskad"
    fi
...
show_menu() {
    while true; do
        ...
        echo -e "${CYAN}"
        echo "======================================================"
        echo "         Port Forwarding Manager - iptables          "
        echo "======================================================"
        echo -e "${NC}"
